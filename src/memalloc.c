...
/* static unsigned int carefulAlignSize;    /\* Alignment size for machine - 8 bytes for DEC alpha, 4 for Sparc. *\/ */
...
if ((int)sizeof(void *) > carefulAlignSize)
    carefulAlignSize = sizeof(void *);
if ((int)sizeof(long) > carefulAlignSize)
