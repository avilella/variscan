...
int 	__paf_open_read	(SF_PRIVATE *psf)
{	PAF_FMT			paf_fmt ;
	unsigned int	marker ;
	int				error ;
	...
	__psf_sprintf (psf, "Signature   : %D\n", marker) ;

	fread (&paf_fmt, sizeof (PAF_FMT), 1, psf->file) ;
	
	if (__CPU_IS_LITTLE_ENDIAN__ && marker == PAF_MARKER)
		endswap_paf_fmt (&paf_fmt) ;
	else if (__CPU_IS_BIG_ENDIAN__ && marker == FAP_MARKER)
		endswap_paf_fmt (&paf_fmt) ;

	__psf_sprintf (psf, "Version     : %d\n", paf_fmt.version) ;
	if (paf_fmt.version != 0)
	{	__psf_sprintf (psf, "*** Bad version number. Should be zero.\n") ;
		return SFE_PAF_VERSION ;
		} ;
		
	__psf_sprintf (psf, "Endianness  : %d => ", paf_fmt.endianness) ;
	if (paf_fmt.endianness)
		__psf_sprintf (psf, "Little\n", paf_fmt.endianness) ;
	else
		__psf_sprintf (psf, "Big\n", paf_fmt.endianness) ;
	...
	__psf_sprintf (psf, "Format      : %d => ", paf_fmt.format) ;

	switch (paf_fmt.format)
	{	case  0 :	__psf_sprintf (psf, "16 bit linear PCM\n") ;
		...
		case  1 :	__psf_sprintf (psf, "24 bit linear PCM\n") ;
		...
		default :   __psf_sprintf (psf, "Unknown\n") ;
					return SFE_PAF_UNKNOWN_FORMAT ;
					break ;
		} ;

	__psf_sprintf (psf, "Channels    : %d\n", paf_fmt.channels) ;
	__psf_sprintf (psf, "Source      : %d => ", paf_fmt.source) ;
	
	switch (paf_fmt.source)
	{	case  1 : __psf_sprintf (psf, "Analog Recording\n") ;
					break ;
		case  2 : __psf_sprintf (psf, "Digital Transfer\n") ;
					break ;
		case  3 : __psf_sprintf (psf, "Multi-track Mixdown\n") ;
					break ;
		case  5 : __psf_sprintf (psf, "Audio Resulting From DSP Processing\n") ;
					break ;
		default : __psf_sprintf (psf, "Unknown\n") ;
					break ;
		} ;
...
int 	__paf_open_write	(SF_PRIVATE *psf)
{	PAF_FMT			paf_fmt ;
	int				format, subformat, error ;
	...
	/* Audio data starts at byte 2048. Seek to that position and
	** fseek () will fill unused bytes with zero.
	*/
	fseek (psf->file, 2048, SEEK_SET) ;
...
{	PAF24_PRIVATE	*ppaf24 ; 
	...
	{	__psf_sprintf (psf, "*** Warning : file seems to be truncated.\n") ;
...
{	int				k, *iptr, newsample, channel ;
	...
	if ((__CPU_IS_BIG_ENDIAN__ && psf->endian == SF_ENDIAN_LITTLE) ||
			(__CPU_IS_LITTLE_ENDIAN__ && psf->endian == SF_ENDIAN_BIG))
...
{	PAF24_PRIVATE 	*ppaf24 ; 
	...
	int				k, bufferlen, readcount = 0, count ;
	int				index = 0, total = 0 ;
double		normfact ;/* EMMANUEL */
	normfact = (normalize ? 1.0 / ((double) (1<<23)) : 1.0) ; /* EMMANUEL */
	...
	{	readcount = (len >= bufferlen) ? bufferlen : len ;
		count = paf24_read (psf, ppaf24, iptr, readcount) ;
		for (k = 0 ; k < readcount ; k++)
			ptr [index+k] = (double) (iptr [k])*normfact ; /* EMMANUEL */
		index += readcount ;
		total += count ;
		len -= readcount ;
		} ;
