...
{	COMM_CHUNK	comm_fmt ;
	...
	{	fread (&marker, sizeof (marker), 1, psf->file) ;
		if (marker == FORM_MARKER) {
         ...
         else if (marker == AIFC_MARKER || marker == AIFF_MARKER) {
					if (parsestage != 1)
						return SFE_AIFF_NO_FORM ;
					filetype = marker ;
					__psf_sprintf (psf, " %D\n", marker) ;
			...
			else if (marker ==  NAME_MARKER  || marker == AUTH_MARKER  || marker == ANNO_MARKER  || marker == c_MARKER  || marker == FVER_MARKER) {
					if (parsestage < 2)
						return SFE_AIFF_NO_FORM ;
					fread (&dword, sizeof (dword), 1, psf->file) ;
			...
			else if (marker ==  MARK_MARKER  || marker == INST_MARKER || marker == APPL_MARKER) {
					if (parsestage < 2)
						return SFE_AIFF_NO_FORM ;
					fread (&dword, sizeof (dword), 1, psf->file) ;
					dword = BE2H_INT (dword) ;
...
**	it has a copyright which probably prevents it from being allowable as GPL.
*/
