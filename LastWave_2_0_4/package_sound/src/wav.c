...
{	WAV_FMT			wav_fmt ;
	...
	{	fread (&marker, sizeof (marker), 1, psf->file) ;
		...
		else if (marker ==   cue_MARKER || marker ==  LIST_MARKER || marker ==  INFO_MARKER 
		         || marker ==   smpl_MARKER || marker ==   bext_MARKER || marker ==  MEXT_MARKER 
		         || marker ==   DISP_MARKER) {
					fread (&dword, sizeof (dword), 1, psf->file) ;
					dword = LE2H_INT (dword) ;
					__psf_sprintf (psf, "%D : %d\n", marker, dword) ;
