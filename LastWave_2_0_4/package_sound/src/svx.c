...
static	int		__svx_close	(SF_PRIVATE  *psf) ;
...
int 	__svx_open_read	(SF_PRIVATE *psf)
{	VHDR_CHUNK		vhdr ;
	...
	{	fread (&marker, sizeof (marker), 1, psf->file) ;
		if (marker == FORM_MARKER) {
		 ...
		 else if (marker == SVX8_MARKER || marker ==  SV16_MARKER) {
					if (parsestage != 1)
						return SFE_SVX_NO_FORM ;
					filetype = marker ;
					__psf_sprintf (psf, " %D\n", marker) ;
	     ...
	     else if (marker ==  VHDR_MARKER) {
					if (parsestage != 2)
						return SFE_SVX_NO_FORM ;
					fread (&dword, sizeof (dword), 1, psf->file) ;
					vhdrsize = BE2H_INT (dword) ;

					__psf_sprintf (psf, " VHDR : %d\n", vhdrsize) ;

					fread (&vhdr, sizeof (vhdr), 1, psf->file) ;
					if (__CPU_IS_LITTLE_ENDIAN__)
						endswap_vhdr_chunk (&vhdr) ;

					__psf_sprintf (psf, "  OneShotHiSamples  : %d\n", vhdr.oneShotHiSamples) ;
					__psf_sprintf (psf, "  RepeatHiSamples   : %d\n", vhdr.repeatHiSamples) ;
					__psf_sprintf (psf, "  samplesPerHiCycle : %d\n", vhdr.samplesPerHiCycle) ;
					__psf_sprintf (psf, "  Sample Rate       : %d\n", vhdr.samplesPerSec) ;
					__psf_sprintf (psf, "  Octave            : %d\n", vhdr.octave) ;

					__psf_sprintf (psf, "  Compression       : %d => ", vhdr.compression) ;
					
					switch (vhdr.compression)
					{	case 0 : __psf_sprintf (psf, "None.\n") ;
								break ;
						case 1 : __psf_sprintf (psf, "Fibonacci delta\n") ;
								break ;
						case 2 : __psf_sprintf (psf, "Exponential delta\n") ;
								break ;
						} ;

					__psf_sprintf (psf, "  Volume            : %d\n", vhdr.volume) ;
					...
					{	__psf_sprintf (psf, " BODY : %d (should be %d)\n", psf->datalength, psf->filelength - psf->dataoffset) ;
						psf->datalength = psf->filelength - psf->dataoffset ;
						} 
					else
						__psf_sprintf (psf, " BODY : %d\n", psf->datalength) ;

					fseek (psf->file, psf->datalength, SEEK_CUR) ;
					parsestage = 4 ;
	     }
	     
	     else if (marker == NAME_MARKER || marker ==  ANNO_MARKER) {
					if (parsestage < 2)
						return SFE_SVX_NO_FORM ;
					fread (&dword, sizeof (dword), 1, psf->file) ;
		...
		else if (marker ==  AUTH_MARKER  || marker ==  c_MARKER || marker ==  CHAN_MARKER) {
					if (parsestage < 2)
						return SFE_SVX_NO_FORM ;
					fread (&dword, sizeof (dword), 1, psf->file) ;
					dword = BE2H_INT (dword) ;
					...
					else if ((dword = ftell (psf->file)) & 0x03)
	...
	psf->close = (func_close) __svx_close ;
...
int 	__svx_open_write	(SF_PRIVATE *psf)
{	static	char 	annotation	[] = "libsndfile by Erik de Castro Lopo\0\0\0" ;
	...
	__psf_hprintf (psf, "mL"  , FORM_MARKER, FORMsize) ;
	__psf_hprintf (psf, "m"   , (psf->bytewidth == 1) ? SVX8_MARKER : SV16_MARKER) ;
	__psf_hprintf (psf, "mLb" , VHDR_MARKER, sizeof (VHDR_CHUNK), &vhdr, sizeof (VHDR_CHUNK)) ;
	__psf_hprintf (psf, "mSmS", NAME_MARKER, psf->filename, ANNO_MARKER, annotation) ;
	__psf_hprintf (psf, "mL"  , BODY_MARKER, psf->datalength) ;
...
int	__svx_close	(SF_PRIVATE  *psf)
{	
	...
	{	/*  Now we know for certain the length of the file we can re-write 
		...
		*/
		...
		__psf_hsetf (psf, FORM_MARKER, "L", psf->filelength - 8) ;
		__psf_hsetf (psf, BODY_MARKER, "L", psf->filelength - psf->dataoffset) ;
