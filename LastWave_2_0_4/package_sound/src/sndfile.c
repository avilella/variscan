...
{	SF_PRIVATE 	*psf ;
	...
	filetype = guess_file_type (psf->file) ;
	fseek (psf->file, 0, SEEK_SET) ;
	switch (filetype)
	{	case	SF_FORMAT_WAV :
		...
		case	SF_FORMAT_PAF :
				sf_errno = __paf_open_read (psf) ;
				break ;

		case	SF_FORMAT_SVX :
				sf_errno = __svx_open_read (psf) ;
				break ;
...
{	SF_PRIVATE 	*psf ;
	...
	{	case	SF_FORMAT_WAV :
		...
		case    SF_FORMAT_PAF :
				if ((sf_errno = __paf_open_write (psf)))
				{	free (psf) ;
					return NULL ;
					} ;
				break ;
		...
		case    SF_FORMAT_SVX :
				if ((sf_errno = __svx_open_write (psf)))
				{	free (psf) ;
					return NULL ;
					} ;
				break ;
...
{	SF_PRIVATE 	*psf ;
	...
	if (! (psf->blockwidth && psf->datalength && psf->dataoffset>=0)) /* ??? EMMANUEL */
